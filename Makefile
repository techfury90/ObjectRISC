...
# capability-value object API), orvec.s (the growable capability array),
# orbuf.s (the growable byte buffer), and doc.s (the document model: Block +
# Document). The value-API ones are bare asm because their ops take `__or`
# params (and the collections loop over them), which would trigger pcc-orisc's
# per-frame OBJSTORE prologue — and that miscompiles here (see the asm files /
# obj_or.h).
LIBORISC_OBJS_S  := $(BUILD)/lib/preempt_handler.oro $(BUILD)/lib/obj_or.oro \
                    $(BUILD)/lib/orvec.oro $(BUILD)/lib/orbuf.oro \
                    $(BUILD)/lib/doc.oro
...
$(BUILD)/lib/orbuf.oro: tools/cc/lib/orbuf.s | $(BUILD)/lib
	$(ASMORISC) -r $< -o $@

$(BUILD)/lib/doc.oro: tools/cc/lib/doc.s | $(BUILD)/lib
	$(ASMORISC) -r $< -o $@
