#!/bin/sh
# test_font_smoke.sh — end-to-end test of WM-mediated vector graphics
# (Phase 59 / WM γ.11).
#
# Mirrors test_wm_smoke.sh exactly; the only difference is the smoke
# program (font_smoke.c instead of wm_smoke.c) and the success line
# (`font_smoke: PASS` vs `wm_smoke: PASS`).
#
...
#   - simorisc CPU 1 running font_smoke.orx (the test client)
#
# font_smoke exercises every VEC_OP_* through the libc vec_*() helpers
...
# --- build font_smoke.orx -------------------------------------------
"$PCC_BUILD/cc/cpp/orisc-unknown-none-cpp" \
    -I tools/cc/arch/orisc -I tools/cc/lib examples/cc/font_smoke.c \
    > "$TMP/sm.i"
"$PCC_BUILD/cc/ccom/orisc-unknown-none-ccom" < "$TMP/sm.i" > "$TMP/sm.s"
python3 tools/asm/asmorisc -r "$TMP/sm.s"                       -o "$TMP/sm.oro"
python3 tools/ld/orld -o "$TMP/font_smoke.orx" \
...
python3 tools/sim/simorisc --connect "$SOCK" --pid 1 \
    --service "0=0@0" --service "0=0@0" --service "0=0@0" \
    --service "18=1@9" \
    "$TMP/font_smoke.orx" >"$TMP/cpu.out" 2>"$TMP/cpu.err" &
CPU=$!
...
echo "--- font_smoke (cpu1) stdout ---"
cat "$TMP/cpu.out"
echo "--- font_smoke (cpu1) stderr ---"
cat "$TMP/cpu.err"
...
grep -q "font_smoke: PASS" "$TMP/cpu.out" \
